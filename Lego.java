...
class Composee extends Piece{
	...
	public String toString(){
		String resultats = "";
		for (Piece p : pieces){
			resultats += p.toString() + ",";
		}
		return super.toString() + " (" + resultats.substring(0, resultats.length() - 2) + ")";
	}
}
...
class Construction{
	...
	public String toString(){
		...
		for(Composant c: composants){
			resultats += c.getPiece().toString() + " (quantite " + c.getQuantite() + ")\n";
		}
		return resultats;
	}
}
...
class Lego {
    ...
    public static void main(String[] args) {
        ...
        fenetre.construire(new Simple("volet", "droite"));
